...
class Sim extends BaseController {
        ...
        if(session()->get("logged_in")){
            ...
            foreach ($list->getResult() as $row) {
                ...
                $val[] = '<div style="text-align: center;">'
                        . '<button type="button" class="btn btn-outline-primary btn-fw" onclick="ganti('."'".$row->idsimulator."'".')">Ganti</button>&nbsp;'
                        . '<button type="button" class="btn btn-outline-danger btn-fw" onclick="hapus('."'".$row->idsimulator."'".','."'".$row->nama_simulator."'".')">Hapus</button>'
                        . '</div>';
                $data[] = $val;
            }
    ...
    public function ajax_add() {
        if(session()->get("logged_in")){
            $data = array(
                'idsimulator' => $this->model->autokode("S","idsimulator","simulator", 2, 7),
                'nama_simulator' => $this->request->getPost('simulator'),
                'letak' => $this->request->getPost('letak'),
                'tahun' => $this->request->getPost('tahun'),
            );
            $simpan = $this->model->add("simulator",$data);
            if($simpan == 1){
    ...
    public function ganti(){
        if(session()->get("logged_in")){
            $kond['idsimulator'] = $this->request->uri->getSegment(3);
            $data = $this->model->get_by_id("simulator", $kond);
    ...
    public function ajax_edit() {
        if(session()->get("logged_in")){
            ...
            $kond['idsimulator'] = $this->request->getPost('kode');
            $update = $this->model->update("simulator",$data, $kond);
            if($update == 1){
    ...
    public function hapus() {
        if(session()->get("logged_in")){
            $kond['idsimulator'] = $this->request->uri->getSegment(3);
            $hapus = $this->model->delete("simulator",$kond);
            if($hapus == 1){
