<script type="text/javascript">
    ...
    $(document).ready(function () {
        table = $('#tb').DataTable({
            ajax: "<?php echo base_url(); ?>/sakitsimnon/ajaxlist",
            ordering: false
        });
    });
    ...
    function add() {
        ...
        $.ajax({
            url: "<?php echo base_url(); ?>/sakitsimnon/load_sim/" + mode,
            type: "GET",
            dataType: "JSON",
    ...
    function save() {
        ...
        if (sim === '-') {
            ...
            var url = "";
            if (save_method === 'add') {
                url = "<?php echo base_url(); ?>/sakitsimnon/ajax_add";
            } else {
                url = "<?php echo base_url(); ?>/sakitsimnon/ajax_edit";
            }
    ...
    function hapus(id, nama) {
        if (confirm("Apakah anda yakin menghapus sakit simulator " + nama + " ?")) {
            $.ajax({
                url: "<?php echo base_url(); ?>/sakitsimnon/hapus/" + id,
                type: "POST",
                dataType: "JSON",
                success: function (data) {
                    alert(data.status);
    ...
    function ganti(id) {
        ...
        $.ajax({
            url: "<?php echo base_url(); ?>/sakitsimnon/ganti/" + id,
            type: "POST",
            dataType: "JSON",
            success: function (data) {
    ...
    function load_sim_ulang1(kode){
        ...
        $.ajax({
            url: "<?php echo base_url(); ?>/sakitsimnon/load_sim1/" + mode + "/" + kode,
            type: "GET",
            dataType: "JSON",
            success: function (data) {
                $('#sim').html(data.status);
    ...
    function detil(kode){
        window.location.href = "<?php echo base_url(); ?>/sakitsimnon/detil/" + kode;
    }
    ...
    function cetak(){
        ...
        $.ajax({
            url: "<?php echo base_url(); ?>/sakitsimnon/load_smulator",
            type: "POST",
            dataType: "JSON",
            success: function (data) {
                $('#simulator_cetak').html(data.status);
            }, error: function (jqXHR, textStatus, errorThrown) {
                alert('Error load simulator');
            }
        });
    }
    
    function printcetak(){
        var simulator_cetak = document.getElementById('simulator_cetak').value;
        ...
        if(tgl1 === ""){
            ...
            window.location.href = "<?php echo base_url(); ?>/sakitsimnon/cetak/" + tgl1 + "/" + tgl2 + "/" + simulator_cetak;
        }
    }
        ...
        <div class="modal-content">
            ...
            <div class="modal-body">
                <form id="form_cetak" class="form-horizontal">
                    <div class="form-group">
                        <label>Simulator</label>
                        <select id="simulator_cetak" name="simulator_cetak" class="form-control">
